...
mod properties {
    ...
    /// Unlike the shaping tests' table, this one keeps `Invalid`: a buffer
    /// accepts it, and only shaping refuses it.
    ...
    fn draw_settings(tc: &hegel::TestCase) -> Settings {
        let settings = Settings {
            ...
            // `.codec("ascii")` keeps the characters below U+0080.
    ...
    fn guess_segment_properties_is_idempotent(tc: hegel::TestCase) {
        let items = draw_items(&tc);
        let mut buffer = filled(&items);
        let preset_direction = tc.draw(generators::optional(
            generators::sampled_from(DIRECTIONS.to_vec()).print_as_debug(),
        ));
        if let Some(direction) = preset_direction {
            buffer.set_direction(direction);
        }
        let preset_script = tc.draw(generators::booleans());
        if preset_script {
    ...
    /// exactly as it arrived". Four of the five variants are provoked here in
    /// turn. `UnusableFont` needs a font with nothing to shape with, and none
    /// is built here.
    #[hegel::test]
