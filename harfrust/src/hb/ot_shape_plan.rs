...
mod tests {
    ...
    mod properties {
        ...
        fn draw_properties(tc: &hegel::TestCase) -> PlanProperties {
            ...
            tc.note(&format!("properties = {properties:?}"));
            properties
        }
        ...
        fn a_key_with_different_segment_properties_does_not_match(tc: hegel::TestCase) {
            ...
            match tc.draw_silent(generators::integers::<u8>().max_value(2)) {
                0 => {
                    other.direction = tc.draw_silent(generators::sampled_from(
                        DIRECTIONS
                            .iter()
                            .copied()
                ...
                1 => {
                    other.script = tc.draw_silent(generators::sampled_from(
                        SCRIPTS
                            .iter()
                            .copied()
                            .map(Some)
                ...
                _ => {
                    other.language = tc.draw_silent(generators::sampled_from(
                        LANGUAGES
                            .iter()
